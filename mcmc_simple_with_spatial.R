...
load(paste0(read_folder,'spatial.RData')) # with param_true,param_name,y,lambda,x
alpha = param_true[1]
beta = param_true[2]
...
likelihood <- function(param,x){
  ...
  return(sum(singlelikelihoods)) # an array of likelihoods for each week
}
...
prior <- function(param,x){
  ...
  return(alpha_prior+beta_prior+sum(x_prior)+p_prior)
}
...
find_beta <- function(beta_){
  ...
  pdf(paste0('trials_mcmc/simple_with_spatial_beta_log(',as.character(exp(beta_)),").pdf"))
  par(mfrow = c(3,3))
  for (i in 1:length(param_name)){
